...
export const initDB = () => {
  ...
  try {
    const checkFeatures = db.prepare("SELECT count(*) as count FROM class_features WHERE class_name = 'Bárbaro'").get() as { count: number };
    if (checkFeatures.count === 0) {
      const insert = db.prepare("INSERT INTO class_features (class_name, feature_name, level_acquired, description) VALUES (?, ?, ?, ?)");
      const transaction = db.transaction((features) => {
        for (const f of features) {
          insert.run(f.class_name, f.feature_name, f.level_acquired, f.description);
        }
      });
      transaction([
      ...
      console.log("⚔️ Seeder de rasgos del bárbaro insertado.");
    }

    const checkFighter = db.prepare("SELECT count(*) as count FROM class_features WHERE class_name = 'Guerrero'").get() as { count: number };
    if (checkFighter.count === 0) {
      const insert = db.prepare("INSERT INTO class_features (class_name, feature_name, level_acquired, description) VALUES (?, ?, ?, ?)");
      const transaction = db.transaction((features) => {
        for (const f of features) {
          insert.run(f.class_name, f.feature_name, f.level_acquired, f.description);
      ...
      console.log("⚔️ Seeder de rasgos del guerrero insertado.");
    }

    const checkRogue = db.prepare("SELECT count(*) as count FROM class_features WHERE class_name = 'Pícaro'").get() as { count: number };
    if (checkRogue.count === 0) {
      const insert = db.prepare("INSERT INTO class_features (class_name, feature_name, level_acquired, description) VALUES (?, ?, ?, ?)");
      const transaction = db.transaction((features) => {
        for (const f of features) {
          insert.run(f.class_name, f.feature_name, f.level_acquired, f.description);
      ...
      console.log("⚔️ Seeder de rasgos del pícaro insertado.");
    }
  } catch (e) {

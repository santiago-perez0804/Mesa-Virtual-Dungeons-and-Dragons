...
interface SkillModifierModalProps {
  character: any;
  socket: any;
  skillLabel: string;
...
export const SkillModifierModal = ({ character, socket, skillLabel, attributeKey, onClose, onUpdate }: SkillModifierModalProps) => {
  ...
  // Proficiency
  const currentSelectedSkills = charInv.habilidades || [];
  const isProficient = currentSelectedSkills.includes(skillLabel);
  ...
  // Custom modifiers from DB for this skill
  const modKey = `custom_skill_${skillLabel}_modifiers`;
    ...
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }} onClick={onClose}>
      <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '30px', width: '400px', maxWidth: '90vw', position: 'relative', display: 'flex', flexDirection: 'column', gap: '20px', boxShadow: '0 10px 40px rgba(0,0,0,0.8)' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <h2 className="font-cinzel" style={{ margin: '10px 0 5px 0', color: 'var(--gold-primary)' }}>Habilidad: {skillLabel}</h2>
          <div className="mono" style={{ fontSize: '3rem', color: totalMod >= 0 ? '#27ae60' : '#e74c3c', fontWeight: 'bold' }}>
            {totalModStr}
          </div>
          <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
            Basada en {attributeLabel}
          </div>
        </div>
        ...
        <form onSubmit={handleAddMod} style={{ display: 'flex', gap: '10px' }}>
          ...
          <input 
            ...
            placeholder="Ej: Pericia, Objeto mágico..."

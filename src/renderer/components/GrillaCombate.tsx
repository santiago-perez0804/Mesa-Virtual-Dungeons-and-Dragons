...
import { Ghost, User, Backpack, Box, Swords, ArrowRight } from 'lucide-react';
...
import { ModalSalud } from './combate/modales/ModalSalud';
import { ModalCofreInventario } from './combate/modales/ModalCofreInventario';
import { ModalDetalleCombatiente } from './combate/modales/ModalDetalleCombatiente';
...
export const CombatGrid = ({ socket, userRole, currentUser, boardTokens, characters, monsters, chatMessages, compendium = [], onOpenCharacterSheet, onOpenMonsterSheet }: any) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg-base)', position: 'relative' }} onClick={() => { setActiveTokenId(null); setContextMenu(null); }}>
      ...
      {selectedChestToken && (
        <ModalCofreInventario
          selectedChestToken={selectedChestToken}
          boardTokens={boardTokens}
          userRole={userRole}
          socket={socket}
          onSlotClick={handleChestSlotClick}
          onLootItem={handleLootItemClick}
          onLootAllCoins={handleLootAllCoins}
          onClose={() => setSelectedChestToken(null)}
        />
      )}

      {compendiumSlotIndex !== null && (
        <ModalSeleccionarObjeto
          compendium={compendium}
          itemSearchQuery={itemSearchQuery}
          setItemSearchQuery={setItemSearchQuery}
          onSelect={handleSelectItemForSlot}
          onClose={() => setCompendiumSlotIndex(null)}
          titulo={<><Box className="w-5 h-5 inline-block mr-2" /> CARGAR OBJETO A COFRE</>}
        />
      )}
      ...
      {healthModalToken && (
        <ModalSalud
          healthModalToken={healthModalToken}
          setHealthModalToken={setHealthModalToken}
          characters={characters}
          socket={socket}
